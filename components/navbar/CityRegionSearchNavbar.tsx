...
import { routerToHotelCityPageWindowObj } from "@/lib/handlers/pageHandler";
import {
  compareDate,
  getDateDifference,
  addDays,
...
export const CityRegionSearchNavbar = (props: Props) => {
  ...
  // extract the selected hotel data from props
  const [selectedHotel, setselectedHotel] = useState(
    props.search_title != ""
      ? props.search_title
      : "Search Hotel / City / Region",
  );
  // extract citySlug from the router query
  const city: any =
    typeof router.query?.citySlugName === "string"
      ? router.query.citySlugName
      : "hotels-in-India";
  const [cityName, setCityName] = useState<string>(city.split("-").pop());
  ...
  useEffect(() => {
    ...
    setCityName(capitalizeWords(newCity.split("-").pop()));
  }, [router.query]);
  ...
  // close calender model on clicking outside
  ...
  // close guest model on clicking outside
  ...
  // recommendation list close on clicking outside
  ...
  useEffect(() => {
    setIsDateSelected(router.query?.checkin ? true : false);
    setselectedHotelCitySlug(router.query.citySlugName || "");
  }, [router.query]);
  ...
  const isSearchButtonDisabled = child_age && child_age.every((age) => age.age !== 0);
  ...
  const searchHandler = () => {
    ...
    if (
      regionSplitSlugName === undefined &&
      router.query.cityRegionSlugName !== undefined
    ) {
      regionSplitSlugName = Array.isArray(router.query.cityRegionSlugName)
        ? router.query.cityRegionSlugName[0]
        : router.query.cityRegionSlugName;
    }
            ...
            <div className="flex items-center gap-x-4">
              <div className="relative">
                <div className="flex w-full flex-col space-y-1">
                  ...
                  <input
                    className="w-56 whitespace-nowrap rounded-lg border-0 bg-black/25 px-4 py-2 font-medium text-white outline-none placeholder:text-gray-400 xl:w-72"
                    disabled={false}
                    value={searchHotelInput}
                    onChange={(e) => {
                      setSearchHotelInput(e.target.value);
