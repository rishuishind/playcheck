...
import Dropdown from "@/components/visa/dropdown";
import Form from "@/components/visa/form";
import visaLadingImage from "@/public/visa-landing.webp";
import Content from "@/components/visa/content";
import Evisa from "@/components/visa/eVisa";
import Accordion from "@/components/visa/accordion";
// const PopularPackage = await dynamic(() => import("./popularPackages"));
const Sampleformvisa = dynamic(() => import("@/components/visa/sampleVisa"), {
  ssr: false,
});
...
const { LOGO_IMAGE_URL1 } = await import("@/lib/helper");
import { Toaster } from "sonner";
import Link from "next/link";
import Image from "next/image";
import { EVISA_URL } from "@/lib/helper";
const Countries = dynamic(() => import("@/components/visa/eligibleCountries"), {
  ssr: true,
});
import Navbar from "@/components/navbar/Navbar";

export const visaLandingFirstFaq = (
  <div>
    ...
    <div className="mb-2 w-full">
        ...
        <tbody>
          <tr>
            <td className="border border-gray-400 px-4 py-2">
              <ul className="list-decimal text-ellipsis pl-4">
                <li>Angola</li>
                <li>Benin</li>
                <li>Burkina Faso</li>
                <li>Burundi</li>
            ...
            <td className=" flex h-full items-start px-4 py-2">
      ...
      <p className="mt-2 font-bold">
        This list was last updated on 3rd January 2023 by WHO.
      </p>
    </div>
  </div>
);

export const visaLandingSecondFaq = (
  <ul className="list-disc pl-4">
    ...
    <li>A recent photograph of the traveller</li>
    <li>
      The digital photograph to be uploaded along with the e-Visa application
...
export const visaLandingAccordionItems = [
  ...
  return (
    <>
      <VisaHead
        ...
        canonicalUrl={`https://staybook.in${EVISA_URL}`}
        metaTitle="Apply for Indian e-Visa Online in Just 05 min, Application Form"
        metaDescription="Apply for Indian travel e-Visa online allows citizens from over 160 countries for various travel purposes, including tourism, business, medical treatment, and conferences. Fill the Application for e-visa 4 days before the travel date."
      />
      <Toaster />

      <Navbar />

      <div className="relative h-[20vh] overflow-hidden bg-gray-400 md:h-[25vh] lg:h-[40vh]">
        <div className="absolute inset-0 flex before:absolute before:inset-0">
          <Image
            className="contrast-less object-cover brightness-50 saturate-150"
            src={visaLadingImage}
            alt="Landing Image"
            width={1920}
            height={120}
        ...
        <div className="wrapper relative">
          <h1 className="pt-7 text-5xl font-bold text-white sm:text-xl md:text-6xl lg:text-7xl">
            Apply Indian eVisa
          </h1>
        </div>
      </div>
      ...
      <div className="h-full" style={{ backgroundColor: "#f0f1f5" }}>
        {/* starting info */}
        <div className="wrapper flex flex-col gap-4 xl:flex-row">
          <div style={{ backgroundColor: "#f0f1f5" }} className="h-full">
            {/* dropdown */}
            <Dropdown />

            <Evisa2 handleClick={handleFormOpen} />
          ...
          {breadcrumb.map((link: string, index: number) => (
            <div key={link} className="font-medium">
              <Link
                ...
                className="text-sm tracking-wide text-blue-700 hover:text-primary"
              >
                {link}
              </Link>
